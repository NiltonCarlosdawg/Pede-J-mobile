...
function OrderItemCard({
  ...
  const hasDriverRating = useAppSelector((state) =>
    Boolean(selectDriverRatingByOrder(state, order.id)),
  );
    ...
    <View style={[styles.orderCard, isActive && styles.activeCard]}>
      ...
      {isActive && (
        <TouchableOpacity
          ...
          onPress={() =>
            router.push({ pathname: '/(tabs)/rastreamento', params: { orderId: order.id } })
          }
        >
        ...
        <View style={{ flexDirection: 'row', gap: spacing.sm, marginTop: spacing.sm }}>
          {!hasDriverRating && (
            <TouchableOpacity
              ...
              hitSlop={ROW_HIT_SLOP}
              onPress={() =>
                router.push({ pathname: '/avaliacao-entregador', params: { orderId: order.id } })
              }
            >
...
export default function OrdersScreen() {
  ...
  const sections = useMemo(() => {
    ...
    if (activeOrders.length > 0)
      result.push({ title: 'Em andamento', data: activeOrders, isActive: true });
    if (pastOrders.length > 0)
      result.push({ title: 'Histórico', data: pastOrders, isActive: false });
    return result;
  ...
  const keyExtractor = useCallback(
    (item: (typeof flatData)[0], index: number) =>
    ...
    return (
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyText}>Faça seu primeiro pedido e ele aparecerá aqui.</Text>
          <TouchableOpacity
            style={{
              backgroundColor: themeColors.primary[500],
              paddingVertical: 12,
