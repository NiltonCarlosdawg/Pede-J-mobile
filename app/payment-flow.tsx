...
import { useGetPaymentQuery, useInitiatePaymentMutation } from '../src/hooks/useApi';
...
export default function PaymentFlowScreen() {
  ...
  const [step, setStep] = useState<'confirm' | 'phone' | 'processing' | 'awaiting' | 'success'>(
    'confirm',
  );
  ...
  function handleConfirm() {
    ...
    initiateBackendPayment().catch((err) => {
      ...
      setError(
        'Não foi possível iniciar o pagamento no servidor. Verifique a ligação e tente novamente.',
      );
      setStep('confirm');
    });
  }
  ...
  function renderConfirmStep() {
    return (
      <>
        ...
        <Button
          title={
            paymentMethod?.type === 'multicaixa_express' || paymentMethod?.type === 'facipay'
              ? 'Continuar'
              : 'Confirmar Pagamento'
          }
          onPress={
          ...
          disabled={
            (paymentMethod?.type !== 'multicaixa_express' &&
              paymentMethod?.type !== 'facipay' &&
              pin.length < 4) ||
            !paymentMethod
          }
        />
  ...
  function renderPhoneStep() {
    ...
    return (
      <>
        ...
        <Text
          style={{ ...typography.labelLg, color: colors.neutral[700], marginBottom: spacing.sm }}
        >
            ...
            >
              ...
              <View style={{ gap: spacing.xs }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  ...
                  <Text
                    style={{ ...typography.bodySm, fontWeight: '700', color: colors.onSurface }}
                  >
                ...
                >
                  {isFacipay
                    ? 'Aguardando confirmação no app FaciPay...'
                    : 'Aguardando resposta do Multicaixa Express...'}
                </Text>
              </View>
            </View>
          </View>
      ...
      <View style={styles.awaitingContainer}>
        ...
        <View style={{ marginTop: spacing.lg, width: '100%', paddingHorizontal: spacing.sm }}>
          ...
          >
            ...
            <View style={{ gap: spacing.xs }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ ...typography.bodySm, color: colors.neutral[600] }}>Entidade</Text>
                <Text style={{ ...typography.bodySm, fontWeight: '700', color: colors.onSurface }}>
                  {ENTITY_CODE} - {ENTITY_NAME}
                </Text>
              </View>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ ...typography.bodySm, color: colors.neutral[600] }}>Valor</Text>
                <Text
                  style={{ ...typography.bodySm, fontWeight: '700', color: colors.primary[500] }}
                >
                  {formatPrice(order?.total ?? 0)}
                </Text>
              </View>
        ...
        <View style={styles.stepsList}>
          ...
          <View style={[styles.stepItem, timer < 170 ? styles.stepItemActive : {}]}>
            ...
            <Text style={styles.stepText}>
              {isFacipay
                ? 'Notificação recebida no FaciPay'
        ...
        >
          {isFacipay
            ? 'A aprovação será recebida automaticamente do FaciPay'
            : 'A aprovação será recebida automaticamente do Multicaixa Express'}
        </Text>
      </View>
    );
  }
    ...
    return (
      <View style={styles.successContainer}>
        ...
        <Text style={styles.successText}>
          O pagamento de {formatPrice(order?.total ?? 0)} via {paymentMethod?.label} foi processado
          com sucesso.
        </Text>
        <Button
          title="Ver Pedido"
          onPress={() =>
            router.replace({
              ...
              params: { orderId, total: String(order?.total) },
            })
          }
        />
      </View>
